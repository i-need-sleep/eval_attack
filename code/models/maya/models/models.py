...
class VictimBLEURT(VictimModel):
    ...
    def __init__(self, args):
        ...
        # Args
        checkpoint = f'lucadiliello/{args.bleurt_checkpoint}'
        self.goal_direction = args.goal_direction
        self.goal_abs_delta = args.goal_abs_delta
        if self.goal_direction != 'down':
    ...
    def set_ref(self, mt, ref):
        self.ref = ref
        self.original_score = self.get_scores([mt])[0]

    def get_scores(self, text_inputs):
        out = [] # [score, ...]
    ...
    def __call__(self, sentences):
        ...
        scores = self.get_scores(sentences)
        
        # Pseudo probs
        probs = []
        for score in scores:
            # Assuming score is between [-2, 2]
            probs.append([{2 - score} / 4, (score + 2) / 4])

        # Success status
        labels = []
        for score in scores:
            if self.original_score - score > self.goal_abs_delta:
                labels.append(1)
            else:
                labels.append(0)
        return self.outputs(None, torch.tensor(probs), torch.tensor(labels))

...
def visualise(args):
    ...
    for i in range(len(df)):
        if Levenshtein.distance(df['mt'][i].split(' '), df['adv'][i].split(' ')) <= args.min_edit_dist:
            continue
        spans_mt, spans_adv, changed = make_highlight_spans(df['mt'][i].split(' '), df['adv'][i].split(' '))
        content += [
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--file_path', default='3-6/base_2017-da_bleurt_down_0.2_gpt.csv', type=str)
